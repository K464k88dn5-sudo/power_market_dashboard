...
if ld:
    _pred_dir_kpi = get_disclosure_pred_dir()
...
with col2:
    ...
    with st.container(border=True):
        # 获取实际观测时间
        _temp_obs_ts = ""
        if not city_temps.empty and "观测时间" in city_temps.columns:
            _obs_times = city_temps["观测时间"].dropna().unique()
            if len(_obs_times) > 0:
                _temp_obs_ts = str(_obs_times[0])[:16].replace("T", " ")
        _temp_ts_str = f" · {_temp_obs_ts}" if _temp_obs_ts else ""
    ...
    with st.container(border=True):
        ...
        if _all_dates:
            ...
            if has_data:
                ...
                # 数据源路径
                _pred_dir = get_disclosure_pred_dir()
                _actual_dir = get_disclosure_actual_dir()
